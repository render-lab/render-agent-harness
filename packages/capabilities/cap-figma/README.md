...
Figma capability pack for the [Render Agent Harness](https://github.com/render-lab/render-agent-harness). **First pack with granular per-action OAuth scopes** (post-Nov-2025 Figma platform update).
...
In the [Figma Developer Apps page](https://www.figma.com/developers/apps), create an OAuth app:
...
End users connect via the operator UI's Connections tab. `fetchAccountLabel` pulls the user's email so the tab shows "Connected as user@example.com".
