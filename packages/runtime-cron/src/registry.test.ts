...
describe("runCronFromRegistry — agent lookup", () => {
  it("rejects an empty agentId with a list of known agents", async () => {
    ...
    await expect(
      runCronFromRegistry({ agents, agentId: "" }),
    ).rejects.toThrow(/agentId is empty.*chat, nightly/);
  });

  it("rejects an unknown agentId with a list of known agents", async () => {
    const agents = { chat: fakeAgent("chat") };
    await expect(
      runCronFromRegistry({ agents, agentId: "missing" }),
    ).rejects.toThrow(/agent "missing" not found.*chat/);
  });

  it("rejects an empty registry with (none) hint", async () => {
    await expect(
      runCronFromRegistry({ agents: {}, agentId: "anything" }),
    ).rejects.toThrow(/Known: \(none\)/);
  });
});
