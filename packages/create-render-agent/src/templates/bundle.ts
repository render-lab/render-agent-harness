...
// Runtime-entry templates moved to @render-harness/registry so the
// harness's agent-add route can drop the same source into an existing
// managed repo when a newly added agent introduces a runtime kind the
// project doesn't yet have. Re-exported here so the existing scaffold
...
} from "@render-harness/registry/runtime-entry-templates";
