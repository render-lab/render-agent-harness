---
title: Ways to Use Render Harness
description: Choose between the CLI, public browse/new site, deployed operator UI, and source-level APIs.
---

Render Harness has several user surfaces. They serve different stages of the agent lifecycle: creating a project, deploying it, operating it, and extending the framework.

```mermaid
flowchart LR
  user["End user"]
  author["Agent author"]
  operator["Operator"]
  integrator["Integrator"]
  contributor["Framework contributor"]

  browse[["Public /browse"]]
  wizard[["Public /new"]]
  cli[["CLI scaffolder"]]
  catalog[/"Template catalog"/]
  ui[["Operator UI"]]
  api[["HTTP API"]]
  registry[["Registry APIs"]]

  user --> browse
  user --> wizard
  author --> cli
  author --> catalog
  operator --> ui
  integrator --> api
  contributor --> registry
  contributor --> catalog

  class user,author,operator,integrator,contributor role
  class browse,wizard,cli,ui,api,registry surface
  class catalog config
  classDef role fill:#050505,stroke:#8a8a8a,color:#d0d0d0,stroke-dasharray:4 3
  classDef surface fill:#101010,stroke:#a855f7,stroke-width:2px,color:#ffffff
  classDef config fill:#050505,stroke:#60a5fa,color:#ffffff
```

## Choose a surface

| Surface | Use it to | Where it lives |
| --- | --- | --- |
| CLI scaffolder | Create a new local project from a blank prompt or starter template. | `packages/create-render-agent` |
| Public `/browse` | Browse official templates and community harness submissions. | `packages/wizard` |
| Public `/new` | Create a managed GitHub repo and deployment flow from a web UI. | `packages/wizard` |
| Operator UI | Chat with deployed agents, inspect runs, view agents, edit config, and read in-product guides. | `packages/ui` mounted by `packages/web` |
| HTTP API | Create runs, stream results, send conversation turns, cancel work, and inspect usage. | `packages/web` |
| Template catalog | Store starter agents and bundles consumed by the CLI and wizard. | `gallery/` |
| Registry APIs | Load manifests, resolve capabilities, and emit Render infrastructure. | `packages/registry` |

## CLI scaffolder

Use the CLI when you want a project on your machine.

```sh
pnpm dlx create-render-agent my-agent
```

In this repo, local development commonly uses `--harness-root` so the scaffolded project links to the local workspace packages:

```sh
pnpm --filter create-render-agent dev
create-render-agent my-agent --harness-root /path/to/render-harness
```

The CLI loads the template catalog, asks which starter to use, writes the project files, and prints local next steps.

Use the CLI when:

- You want to inspect and edit generated files immediately.
- You want to test framework changes against a newly scaffolded project.
- You want a local project before creating a GitHub repo.

## Public browse and new routes

Use the public wizard site when you want to discover harnesses or create a managed repo and deploy path.

The `/browse` route shows official templates from the template catalog alongside community registry submissions. The `/new` route starts the guided scaffolding flow.

The public site is the right surface when:

- You want a no-code or low-code onboarding flow.
- You want a managed GitHub repo created for the user.
- You want a deploy link instead of a local-only project.
- You want to compare official templates with community harnesses before starting.

The public site is implemented as a Hono server plus React SPA in `packages/wizard`.

```mermaid
flowchart LR
  person["User or author"]
  browse[["/browse"]]
  wizard[["/new"]]
  catalog[/"Template catalog"/]
  community[/"Community registry index"/]
  repo[("Managed GitHub repo")]
  deploy[["Deploy to Render link"]]

  person --> browse
  person --> wizard
  browse --> community
  browse --> catalog
  wizard --> catalog
  wizard --> repo
  repo --> deploy

  class person role
  class browse,wizard,deploy surface
  class catalog,community config
  class repo data
  classDef role fill:#050505,stroke:#8a8a8a,color:#d0d0d0,stroke-dasharray:4 3
  classDef surface fill:#101010,stroke:#a855f7,stroke-width:2px,color:#ffffff
  classDef config fill:#050505,stroke:#60a5fa,color:#ffffff
  classDef data fill:#050505,stroke:#f59e0b,color:#ffffff
```

## Template catalog

The template catalog is currently named `gallery/`. It is source data consumed by the CLI, `/new`, and the official entries shown on `/browse`.

The catalog includes:

- `gallery/index.yaml`: The list of available starters.
- `gallery/agents/*/render-harness.yaml`: The starter manifests.
- Optional `README.md` files for template previews.
- Optional `src/*` files for sealed bundles.

The current starters are:

- `chat`
- `support-bot`
- `research-cron`
- `chief-of-staff`

The Chief of Staff template is a sealed multi-agent bundle. It ships multiple agents and source files together instead of generating a single prompt-based agent.

## Deployed operator UI

The operator UI belongs to a specific deployed harness. Scaffolded projects mount it at the service root when UI is enabled. Library users can mount it elsewhere, including `/ui`, by passing a custom `ui.path`.

Use it after deployment to:

- Chat with an agent.
- Resume conversations.
- Inspect runs, messages, tool calls, and tool results.
- View loaded agents and skipped built-ins.
- View usage rollups.
- Edit supported model/config values through the Config tab.
- Read the in-product Guide.

The UI is not the public wizard site. It operates one harness that is already deployed.

```mermaid
flowchart LR
  operator["Operator"]
  ui[["Operator UI"]]
  chat["Chat"]
  runs["Runs"]
  agents["Agents"]
  config["Config"]
  usage["Usage"]

  operator --> ui
  ui --> chat
  ui --> runs
  ui --> agents
  ui --> config
  ui --> usage

  class operator role
  class ui surface
  class chat,runs,agents,config,usage surfaceDetail
  classDef role fill:#050505,stroke:#8a8a8a,color:#d0d0d0,stroke-dasharray:4 3
  classDef surface fill:#101010,stroke:#a855f7,stroke-width:2px,color:#ffffff
  classDef surfaceDetail fill:#101010,stroke:#38bdf8,color:#ffffff
```

## HTTP API

The web package exposes the production HTTP surface. Use it when you want to integrate an external app, custom frontend, Slack bot, webhook, or internal tool.

Common API actions include:

- Create a run.
- Stream a run.
- Create a conversation.
- Append a conversation message.
- Cancel a run.
- Send input to a paused run.
- List agents and usage.

The web service usually fronts a worker so requests return quickly while jobs continue in the background.

```mermaid
flowchart LR
  app["Custom app, bot, or webhook"]
  api[["HTTP API"]]
  runs["Runs"]
  conversations["Conversations"]
  streams["SSE streams"]
  operations["Cancel and HITL input"]

  app --> api
  api --> runs
  api --> conversations
  api --> streams
  api --> operations

  class app role
  class api surface
  class runs,conversations,streams,operations surfaceDetail
  classDef role fill:#050505,stroke:#8a8a8a,color:#d0d0d0,stroke-dasharray:4 3
  classDef surface fill:#101010,stroke:#a855f7,stroke-width:2px,color:#ffffff
  classDef surfaceDetail fill:#101010,stroke:#38bdf8,color:#ffffff
```

## Registry and framework APIs

Use source-level APIs when you are extending the framework itself.

Important registry responsibilities include:

- Parse and validate `render-harness.yaml`.
- Load starter templates from source or bundled snapshots.
- Resolve capability packs.
- Build `AgentDefinition` objects.
- Emit `render.yaml` Blueprints.
- Plan non-Blueprint resources such as Workflow services.

Most application authors should start with the CLI or wizard. Framework contributors usually work directly with the registry, runtimes, and core packages.

## Lifecycle map

```mermaid
flowchart LR
  pick["Pick a starter"]
  author["Author agent behavior"]
  manifest[/"Declare render-harness.yaml"/]
  build[/"Emit render.yaml"/]
  deploy["Deploy"]
  operate["Operate"]
  extend["Extend"]

  pick --> author
  author --> manifest
  manifest --> build
  build --> deploy
  deploy --> operate
  extend -.-> pick
  extend -.-> build

  class pick,author,deploy,operate stage
  class manifest,build config
  class extend extension
  classDef stage fill:#101010,stroke:#a855f7,color:#ffffff
  classDef config fill:#050505,stroke:#60a5fa,color:#ffffff
  classDef extension fill:#111827,stroke:#22c55e,color:#ffffff
```

| Stage | Primary surface |
| --- | --- |
| Pick a starting point | CLI scaffolder or public `/browse` |
| Author agent behavior | Generated TypeScript files |
| Declare deployment shape | `render-harness.yaml` |
| Build Render infrastructure | Registry and Blueprint emitter |
| Start runs | Web API, Cron, Worker, or Workflows |
| Operate deployed agents | The deployed harness's operator UI |
| Extend the framework | Core, registry, runtime, and capability packages |
