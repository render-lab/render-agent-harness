---
title: Operator UI
description: Use a deployed loop's operator UI to chat, inspect runs, view agents, manage config, and read guides.
---

The operator UI is an optional React SPA mounted by the web service. Scaffolded projects mount it at the service root, and library users can choose another path. It is for operating a deployed loop, not for creating a new loop.
...
Passing `ui: true` mounts the UI at `/ui`. Scaffolded projects pass `ui: { path: "/" }` when the user enables the UI, so the deployed loop opens directly into the operator console. To customize the mount path, pass an object:
...
The Vitals tab is an opt-in Render observability surface. It shows Render service instances, CPU, memory, HTTP p95 latency, and recent logs across every service in this loop (web, worker, cron, and any siblings sharing the same Render environment).

A service picker at the top of the tab switches between the loop's services. The web service the operator UI is itself running on is highlighted as the current service.
