...
- A public browse surface for official templates and community harness submissions.
- An operator UI for each deployed harness, usually mounted at the service root in scaffolded projects.
...
- Use `/browse` on the public wizard site to compare official templates and community harness submissions.
...
- Use each deployed harness's operator UI to chat, inspect runs, view agents, edit config, and read guides.
...
| Public `/browse` | Lists official templates and community harness submissions. |
| Public `/new` | Creates a manual guided repo/deploy flow. Browse entries can seed it with a template. |
| Deployed operator UI | Operates one running harness deployment. Scaffolded projects mount it at the service root. |
