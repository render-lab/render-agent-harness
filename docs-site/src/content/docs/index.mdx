...
Render Loops is a TypeScript agent runtime for building agents that run on Render primitives: Web Services, Workers, Cron Jobs, Workflows, Postgres, and Key Value.

The Render Loops model is designed and optimized for Render service shapes. It maps agent work to the Render service that fits it: request entrypoints run through web services, queue-backed work runs on workers, scheduled work runs on Cron Jobs, durable long-running work runs on Workflows, and shared state lives in Render Postgres and Key Value.

Render Loops gives agent projects a shared execution loop, durable state, runtime-specific entrypoints, tool wiring, scaffolding, and an operator UI without asking authors to reassemble those Render services by hand.

A single deployment is called a **loop**: one repo, one Blueprint, one shared Postgres + Key Value, hosting one or more agents. Browse loops, scaffold new ones, and deploy them to Render.

Render Loops is extensible by design. The included agents, templates, examples, and capability packs are starting points: useful as shipped when they match your use case, and meant to be copied, edited, combined, or replaced when they do not. Your product logic lives in agents and capabilities; Render Loops supplies the production substrate around them.

## What you can build

Render Loops supports several agent shapes:
...
These shapes are examples, not limits. You can define new agents, publish new capability packs, add MCP servers, build custom HTTP surfaces, or package multi-agent bundles that share the same underlying loop.

## What a loop gives you

A loop handles the platform work around your agent:
...
- A public browse surface for official templates and community loop submissions.
- An operator UI for each deployed loop, usually mounted at the service root in scaffolded projects.

## The mental model

Think of the project as three structural parts:

- **Loop:** The shared runtime substrate. It owns the model loop, runtime adapters, state, streaming, cancellation, MCP wiring, built-in tools, and Render deployment shapes.
- **Agents:** The behavior layer that runs inside the loop. An agent starts as a `defineAgent()` TypeScript definition, or as a `render-harness.yaml` manifest that resolves to one. It declares the prompt, model, permissions, MCP servers, skills, tools, and runtime triggers.
...
Render Loops is not only a runtime library. It has different surfaces for different jobs:

- Use the CLI scaffolder to create a local project.
- Use `/browse` on the public wizard site to compare official templates and community loop submissions.
- Use `/new` on the public wizard site to create a manual managed repo and deploy flow.
- Use each deployed loop's operator UI to chat, inspect runs, view agents, edit config, and read guides.
- Use the template catalog to define what official starters are available.
...
See [Ways to use Render Loops](/ways-to-use/) for the full map.

## Platform surfaces

Render Loops has four user-facing surfaces:
...
flowchart TB
  ...
  class cli,browse,wizard,registry,ui,api surface
...
| Public `/browse` | Lists official templates and community loop submissions. |
| Public `/new` | Creates a manual guided repo/deploy flow. Browse entries can seed it with a template. |
| Deployed operator UI | Operates one running loop. Scaffolded projects mount it at the service root. |
...
- [Operator UI](/operator-ui/): Operate a deployed loop.
...
- [Operations](/operations/): Debug deployed loops and common failure modes.
